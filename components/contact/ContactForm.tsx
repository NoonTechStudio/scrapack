    ...
    return (
      <div ref={cardRef} className="reveal reveal-delay-1 bg-white rounded-3xl p-8 md:p-10 shadow-lg border border-gray-100">
        <div className="text-center py-10">
          ...
          <h3 className="text-3xl font-black text-[#1A2E1A]">Thank You, {form.fullName.split(' ')[0]}!</h3>
          <p className="text-[#6B7280] mt-3 leading-relaxed max-w-sm mx-auto">
            We have received your quote request and will get back to you within <strong className="text-[#1A2E1A]">24 business hours</strong> with a competitive offer.
          </p>
          ...
          <div className="mt-8 bg-[#FDFAF4] rounded-2xl p-5 text-left max-w-sm mx-auto border border-gray-100">
            ...
            <div className="space-y-2 text-sm">
              {form.company && <div className="flex justify-between"><span className="text-[#6B7280]">Company</span><span className="font-semibold text-[#1A2E1A]">{form.company}</span></div>}
              <div className="flex justify-between"><span className="text-[#6B7280]">Role</span><span className="font-semibold text-[#1A2E1A]">{form.role === 'us-supplier' ? 'US OCC Recycler' : form.role === 'indian-buyer' ? 'Indian Mill/Trader' : form.role === 'broker' ? 'Broker/Agent' : 'Other'}</span></div>
              {form.grade && <div className="flex justify-between"><span className="text-[#6B7280]">Grade</span><span className="font-semibold text-[#1A2E1A]">{form.grade === 'grade-11' ? 'OCC Grade 11' : form.grade === 'grade-12' ? 'OCC Grade 12' : form.grade === 'both' ? 'Grade 11 & 12' : 'Not sure yet'}</span></div>}
              {form.quantity && <div className="flex justify-between"><span className="text-[#6B7280]">Volume</span><span className="font-semibold text-[#1A2E1A]">{form.quantity}</span></div>}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select name="quantity" value={form.quantity} onChange={handleChange} className={fieldClass}>
                  ...
                  <option value="custom">Custom volume — specify in message</option>
                </select>
              </div>
            </div>
          ...
          <>
            <div className="flex flex-col gap-1.5">
              ...
              <textarea name="message" value={form.message} onChange={handleChange} rows={5}
                placeholder="Tell us more about your requirements — moisture tolerance, bale size preferences, loading port, any specific certifications needed, or any other questions you have..."
            ...
            <div className="bg-[#F1F8E9] rounded-2xl p-5 border border-[#2E7D32]/15">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                ...
                {form.grade && <div><span className="text-[#6B7280]">Grade: </span><span className="font-semibold text-[#1A2E1A]">{form.grade === 'grade-11' ? 'OCC Grade 11' : form.grade === 'grade-12' ? 'OCC Grade 12' : form.grade === 'both' ? 'Grade 11 & 12' : 'Not sure'}</span></div>}
